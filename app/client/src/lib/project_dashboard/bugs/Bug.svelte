<script>
    ...
    import { currentUser, userIsProjectLeader, userIsProjectMember, viewedProject, viewedUser, modalControl } from '../../../store';
        ...
        try {
            ...
            if(res.ok) {
                ...
                viewedProject.update(project => {
                    project.bugs = project.bugs.filter(projectBug => projectBug.idBug != bug.idBug);
                    return project;
                });
            } else {
    ...
    const handleMarkBugActive = async() => {
        try {
            ...
            if(res.ok) {
                modalControl.show('success', 'Bug marked as being actively fixed');
                viewedProject.update(project => {
                    for(const markBug of project.bugs) {
                        if(markBug.idBug === bug.idBug) {
                ...
                modalControl.show('error', 'Could not mark the bug as being actively fixed');
            }
        } catch(e) {
            modalControl.show('error', 'Could not mark the bug as being actively fixed');
            console.log(e);
...
<div class="bug-container general-container">
    ...
    <div class="bug-controls">
        ...
        {/if}
        {#if $userIsProjectLeader == true || $currentUser.idUser == bug.submitterId}
